...
public int optimalRoute(int[][] houses) {
  ...
  for (int i = 0; i < cols; i++) { // copy the first row
    dp[0][i] = houses[0][i];
  }
...
private void helper(int[][] houses, int[][] dp, int rows, int cols) {
  ...
  for (int i = 0; i < rows - 1; i++) {
    for (int j = 0; j < cols; j++) {
      if (j >= 1 && j < cols - 1) {
        int left = houses[i][j] + houses[i + 1][j - 1];
        int down = houses[i][j] + houses[i + 1][j];
        int right = houses[i][j] + houses[i + 1][j + 1];
        int maxLeft = Math.max(left, dp[i + 1][j - 1]);
        int maxDown = Math.max(down, dp[i + 1][j]);
        int maxRight = Math.max(right, dp[i + 1][j + 1]);
        dp[i + 1][j - 1] = maxLeft;
        dp[i + 1][j] = maxDown;
        dp[i + 1][j + 1] = maxRight;
        ...
        int down = houses[i][j] + houses[i + 1][j];
        int maxLeft = Math.max(left, dp[i ][j - 1]);
        int maxDown = Math.max(down, dp[i + 1][j]);
        dp[i ][j - 1] = maxLeft;
        dp[i + 1][j] = maxDown;
        ...
        dp[i + 1][j] = houses[i][j] + houses[i + 1][j]; // when we only go down
        dp[i + 1][j + 1]  = houses[i][j] + houses[i + 1][j + 1]; // we are looking at the right side.
      }
    }
    for (int index = 0; index < cols; index++) { // copy the values to our array so we can compute the correct sum
      houses[i + 1][index] = dp[i + 1][index];
    }
  }
}
