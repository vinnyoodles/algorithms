...
public class Solution {
  public int[] findOrder(int numCourses, int[][] prerequisites) {
    ...
    List<Integer> order = topologicalOrder(courses, prerequisites);
    if (order == null) return new int[0];

    return order.toArray(new int[order.size()]);
  }

  public List<Integer> topologicalOrder(int[] courses, int[][] dep) {
